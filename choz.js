...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const totalFitur = () =>{
            var mytext = fs.readFileSync("./Choz.js").toString()
...
user.afkReason = ''
        ...
        case 'enc': {
            if (!isCreator) return newReply(mess.owner)
            if (!q) return newReply(`Contoh ${prefix+command} const Choz = require('Choz-api')`)
...
caption: respon,
sequenceNumber: 1656662972682001, timeOffset: 8600, jpegThumbnail: fs.readFileSync('./media/keqing.jpg'),
contextInfo: {
...
case 'help': {
    //countdown by choco
     // Tanggal target 1 (25 Juli 2023)
  const targetDate1 = new Date('${hbd} 00:00:00');

  // Tanggal target 2 (Idul Adha - 29 Juni 2023)
  const targetDate2 = new Date('${adha} 00:00:00');

  // Menghitung selisih waktu antara tanggal saat ini dengan target 1
  const timeDiff1 = targetDate1.getTime() - new Date().getTime();

  // Menghitung selisih waktu antara tanggal saat ini dengan target 2
  const timeDiff2 = targetDate2.getTime() - new Date().getTime();

  // Menghitung waktu dalam hari, jam, menit, dan detik untuk target 1
  ...
  // Menghitung waktu dalam hari, jam, menit, dan detik untuk target 2
  ...
  ☍ *Bailyes:* choco/Bailyes
  ☍ *Runtime:* ${runtime(process.uptime())}
  ☍ *Total User:* ${Object.keys(global.db.data.users).length}
  ☍ *Total Premium:* ${prem.length}
  ☍ *Total Banned:* ${ban.length}
  ☍ *Total Fitur:* ${totalFitur()}
  ☍ *Hitung Mundur Ultah Owner:* ${days1} hari ${hours1} jam ${minutes1} menit ${seconds1} detik
  ☍ *Hitung Mundur Idul Adha:* ${days2} hari ${hours2} jam ${minutes2} menit ${seconds2} detik
...
conn.sendMessage(m.chat, {
    ...
    contextInfo: {
      externalAdReply: {
        ...
        sourceUrl: "https://chat.whatsapp.com/Ea6jYfiGqWTI34UgH2Wlvq",
        mediaType: 1,
